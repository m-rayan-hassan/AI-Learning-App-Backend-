...
export const SplitScreenSlide: React.FC<SplitScreenSlideProps> = ({ title, bullets, imagePrompt, imageUrl, themeColors }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  const imageSpring = spring({ frame, fps, config: { damping: 14, stiffness: 70 } });
  const imageX = interpolate(imageSpring, [0, 1], [-SLIDE_WIDTH * 0.5, 0]);
  ...
  // Fallback abstract visual from imagePrompt (when no real image)
  ...
  return (
    ...
    >
      ...
      {/* Left Visual section — AI image or gradient fallback */}
      <div
        style={{
          position: 'absolute',
      ...
      >
        {hasImage ? (
          ...
          <AnimatedImage
            ...
            height={SLIDE_HEIGHT - theme.spacing.xl * 2}
            kenBurns="none"
